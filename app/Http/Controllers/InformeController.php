...
class InformeController extends Controller
{
    ...
    public function informeGastos(){
        //Este informe  se encuentra en @DetallesGastosController
    }
    public function imprimirInformeGastos(Request $request){
        $id_empresa = $request->session()->get('id_empresa');
        ...
        $detalle_gastos = DetalleGasto::select('detalles_gastos.id as id','detalles_gastos.id_caja_cierre', 'valor_gasto', 'detalle_gasto', 'cajas.nombre as nombre_caja')
        ...
        $impresora->text($infoEmpresa->direccion."\n");
        $impresora->text("Fecha: ");
        $impresora->text(date(DATE_RFC2822)."\n");
        $impresora->text("\n=====================================\n");
        $impresora->setEmphasis(true);
        $impresora->text(sprintf('%-20s %-8s %+12s', 'GASTO', 'CAJA','PRECIO'));
        $impresora->setEmphasis(false);
        $impresora->text("\n=====================================\n");

        $total=0;
        foreach($detalle_gastos as $dg){
            
            $line = sprintf('%-20s %-8s %12.2f', $dg->detalle_gasto,  $dg->nombre_caja, $dg->valor_gasto);
            $impresora->text($line);
            $impresora->text("\n");    
            $total+=($dg->valor_gasto);
        }
        $impresora->text("\n=====================================\n");
        $impresora->text(sprintf('%+25s %15.2f', 'TOTAL:',  $total));
        $impresora->text("\n");    
        ...
        return redirect()->back()->with("mensaje", "Ticket gastos impreso");        
